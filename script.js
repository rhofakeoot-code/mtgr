...
// تصفير البيانات لإضافة منتجات حقيقية من قاعدة البيانات
const categories = [];
const products = [];
...
// إتاحة الدوال للعمل مع ملف HTML
...
function updateItemQty(productId, change) {
    ...
    if(item) {
        item.qty += change;
        if(item.qty < 1) removeItem(productId); else renderCart();
        updateCartBadge();
    }
}
...
function checkPhoneLength() {
    ...
    btn.style.opacity = (phone && phone.length >= 10) ? '1' : '0.5';
    btn.style.pointerEvents = (phone && phone.length >= 10) ? 'auto' : 'none';
}
